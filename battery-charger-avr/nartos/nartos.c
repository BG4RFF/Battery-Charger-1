...
#endif
static volatile uint16_t taskBuffer[queueLength];/**< Buffer of task IDs */
...
/*---------------------------------------------------------------------------*/
/** @brief Reschedule a suspended task.

This reschedules a task on the queue for execution at an opportune time. If the
...
void taskSchedule(const uint8_t taskID, const uint8_t isr)
{
  ...
  if (! isr)           /* do not re-enable interrupts if calling from ISR. */
    asm("sei\n\t"::);
}
