...
volatile uint16_t scaledChannelValue[4];
...
/** The scaled result according to calibration is also stored for display.*/
            uint16_t channelMulScale = eeprom_read_word((uint16_t *)(channelStart+6));
            uint16_t channelOffScale = eeprom_read_word((uint16_t *)(channelStart+8));
            scaledChannelValue[channelNumber] = rescale(channelValue[channelNumber],
                                                    channelMulScale,channelOffScale);
            if (scaledChannelValue[channelNumber] > 32768) scaledChannelValue[channelNumber] = 0;
...
volatile uint16_t averageBatteryVoltage;
volatile uint32_t currentSum;
volatile uint16_t averagePanelCurrent;
...
is a 16 bit scaled result with the offset added.
*/
...
Note that some outputs on this port overlap with ID read inputs. On a controller
board that has no hardware buffer to protect these pins, disconnect any jumpers
connected to those pins. Leave the UART lines PD0,PD1 alone.

Also set the Power Control register PRR to turn off timers 1&2, SPI and also ADC
...
/* power down unused peripherals - TC1, TC2, SPI, ADC */
